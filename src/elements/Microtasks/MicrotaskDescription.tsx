        ...
        {match(microtask.kind)
          ...
          .with(MicrotaskKinds.CHECK_FACT_RESOURCE, () => (
            <ClassficationTask
              microtask={microtask}
              sentence={sentence}
              taskTitle={`次の青色で強調された事実を表すタスク対象のパラグラフの周囲には，その事実の根拠となる情報源が書かれていますか？選択肢の中から一つ選んで回答してください．`}
            />
          ))
          .with(MicrotaskKinds.CHECK_OPINION_VALIDNESS, () => (
            <ClassficationTask
              ...
              withReason={true}
              reasonText="「書かれている」と回答した場合，「どの文章を読んで書かれていると判断したのか」を簡単に述べてください．「書かれていない」と回答した場合，「どのように文章を改善すれば，より根拠のある意見になるか」を簡単に述べてください．"
            />
          ))
          .exhaustive()}
      </div>
